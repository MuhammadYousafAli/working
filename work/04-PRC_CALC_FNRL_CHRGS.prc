CREATE OR REPLACE PROCEDURE KSHF_ITQA.PRC_CALC_FNRL_CHRGS (
    P_CLNT_SEQ                NUMBER,
    P_INCDNT_DT               DATE,
    P_USER_ID                 VARCHAR2,
    ...
    LOOP
        ...
        --THEN
            ...
            THEN           
                ...
                BEGIN
                    SELECT NVL(SUM (NVL (DUE, 0)) - SUM (NVL (REC, 0)),0) CHRGS,
                       ...
                       NVL(MAX (INST_NUM),0) V_MAX_INST_NUMBER
                ...
                */                           
                ...
                THEN
                    P_BUCKET := 12;
                    IF V_INST_NO_PAID <= 12 THEN V_INST_NO_PAID := 12; END IF;
                ELSIF P_DED_BASE = 2
                THEN
                    P_BUCKET := 6;
                    IF V_INST_NO_PAID <= 6 THEN V_INST_NO_PAID := 6; END IF;
                ELSIF P_DED_BASE = 3
                THEN
                    P_BUCKET := 18;
                    IF V_INST_NO_PAID <= 18 THEN V_INST_NO_PAID := 18; END IF;
                ELSIF P_DED_BASE = 4
                THEN
                    P_BUCKET := 24;
                    IF V_INST_NO_PAID <= 24 THEN V_INST_NO_PAID := 24; END IF;
                ELSIF P_DED_BASE = 5
                THEN
                    P_BUCKET := 0;
                    V_INST_NO_PAID := 0;
                ...
                THEN
                    BEGIN
                        -- CHARGES ABOVE CURRENT TENURE WILL BE DEFFFERED
            ...
            ELSE                    -----------  ELSE PART RANK-2 FOR 2ND LOAN
                IF (RNK.PRD_SEQ = 51)
                THEN
                    PRC_KTK_KSZB_POLICY_RECEIVABLE_RVSL (RNK.LOAN_APP_SEQ,
                                                             P_USER_ID,
                    ...
                    THEN
                        ROLLBACK;
                            P_INCDNT_RTN_MSG_CALC := P_INCDNT_RTN_MSG_CALC;
                    ...
                    THEN
                        ROLLBACK;
                             V_TOT_CHRGS_TO_BE_DEDUCTSUMTOT := 0;
                             P_INCDNT_RTN_MSG_CALC := V_INCDNT_RTN_MSG_DEFF;
                        RETURN;
                    END IF;
                ELSE
                    BEGIN
                        SELECT MIN(PSD.INST_NUM)-1
                              INTO  V_OTHR_INST
                            FROM MW_LOAN_APP  LA
                               JOIN MW_PYMT_SCHED_HDR PSH
                                   ON     LA.LOAN_APP_SEQ = PSH.LOAN_APP_SEQ
...
V_DED_AMT := NVL(V_TOT_CHRGS_TO_BE_DEDUCTSUMTOT,0);
